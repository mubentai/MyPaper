...
对于一般的线性系统，传统的卡尔曼滤波可以通过递推的状态提供一个无偏的最小方差估计，并且能通过估计的协方差值标示估计的不确定度，然而在许多的实际工程应用中，比较典型的系统是动态的非线性系统。首先将非线性系统模型做线性化的处理，然后再根据传统卡尔曼滤波的思想进行处理，以此来实现非线性系统的卡尔曼滤波，则称为扩展卡尔曼滤波(EK\\F)\citeup{liuxintian2011}。扩展卡尔曼滤波是在测量和估计值结果附近进行泰勒级数的展开并且忽略二阶以上的高阶项，得到卡尔曼滤波线性化的方程。扩展卡尔曼滤波对于非线性程度不高的动态系统具有较好的估计效果。
...
(b)无迹卡尔曼滤波-UKF
	...
	无迹卡尔曼滤波是基于UT变换技术运用卡尔曼滤波思想的一种滤波技术。在当前状态预测方程中，UKF运用UT变换来近似的表示k时刻经过非线性变换后在$k\\{\rm{ + }}1$时刻的估计值，借此来处理随机变量X均值和方差的非线性传递。无迹卡尔曼滤波算法\citeup{xiong2006performance}的步骤如下：
...
\pic[htbp]{采样策略结果对比图}{}{3-qiehuanduibi} 
...
\begin{equation}X\left( {k + 1} \right) = f\left[ {X\left( k \right),I\left( k \right)} \right] + u\left( k \right) = \left( {\begin{array}{*{20}{c}}
